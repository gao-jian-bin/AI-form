...
test('does not flash route progress for a fast navigation', async ({ page, request }) => {
...
test('shows route progress for a pending navigation and hides it immediately on completion', async ({ page, request }) => {
  const response = await request.get('/api/topics?pageSize=1')
  const topicPage = await response.json() as { items: Array<{ id: number }> }
  const topic = topicPage.items[0]!
  ...
  expect(visibleDelay).toBeGreaterThanOrEqual(270)

  releaseRequest()
  await navigation
  await expect(page).toHaveURL(new RegExp(`/t/${topic.id}$`))
  await expect(indicator).toHaveCSS('opacity', '0')
  await expect(indicator).toHaveCSS('transition-duration', '0.1s, 0s')
})
