...
const canonicalPath = computed(() => `/t/${topic.value!.id}`)
if (route.path !== canonicalPath.value) {
  await navigateTo({ path: canonicalPath.value, query: route.query, hash: route.hash }, { redirectCode: 301, replace: true })
}
